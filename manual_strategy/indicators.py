...
def get_Price_SMA_Ratio(symbols,dates,lookback=14):
    ...
    #clear out the dataframe to accumulate values into it
    for day in range(price.shape[0]):
        for sym in symbols:
            sma.ix[day,sym]=0

    #calculate SMA in triple-nested for loop
    for day in range(price.shape[0]):
...
def get_Bollinger_Bands_ratio(symbols, dates, lookback):
    ...
    # normalize the price
    price = df / df.values[0]
    sma=get_Price_SMA_Ratio(symbols,dates,lookback)
    bbp=price.copy()
    for day in range(price.shape[0]):
        # loop over the lookback from this day and calculate standard deviation
        for sym in symbols:
            for prev_day in range(day-lookback+1,day+1):
                bbp.ix[day,sym]=bbp.ix[day,sym]+math.pow(price.ix[prev_day,sym] - sma.ix[day,sym],2)

        # finish calculating standard deviation for this day and symbol
        bbp.ix[day,sym]=math.sqrt(bbp.ix[day,sym]/(lookback-1))

        # finish the BB% calculation for this day and symbol
...
def get_helperdata(symbols, dates,lookback): # get helper date for part I plot
    ...
    # calculate the bottom_band and top_band
    for day in range(price.shape[0]):
        # loop over the lookback from this day and calculate standard deviation
        for sym in symbols:
            for prev_day in range(day-lookback+1,day+1):
        ...
        # finish the BB% calculation for this day and symbol
        bottom_band.ix[day,sym]=sma.ix[day,sym]-(2*bbp.ix[day,sym])
        top_band.ix[day,sym]=sma.ix[day,sym]+(2*bbp.ix[day,sym])
