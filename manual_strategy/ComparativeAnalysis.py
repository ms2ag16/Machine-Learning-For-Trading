...
from indicators import get_Price_SMA_Ratio, get_Bollinger_Bands_ratio,get_RSI, get_Momentum, get_MFI
...
if __name__ == "__main__":
    # Part 3
    mbs = MannualStrategy(symbol="JPM", sd=dt.datetime(2010, 1, 1), ed=dt.datetime(2011, 12, 31), sv=100000)
    trades_df = mbs.testPolicy(symbol="JPM", sd=dt.datetime(2010, 1, 1), ed=dt.datetime(2011, 12, 31), sv=100000)
    portval_mbs = mbs.get_manualrule_portfolio(symbol="JPM", sv=100000, sd=dt.datetime(2010, 1, 1),
                                               ed=dt.datetime(2011, 12, 31), commission=9.95, impact=0.005)
    portval_benchmark = mbs.get_benchmark_portfolio(symbol="JPM", sd=dt.datetime(2010, 1, 1),
                                                    ed=dt.datetime(2011, 12, 31), sv=100000, commission=9.95,
                                                    impact=0.005)
    ax = portval_mbs.plot(title="Figure 8. Benchmark vs. Manual Rule-Based Strategy (out of sample)", fontsize=12, color="black",
